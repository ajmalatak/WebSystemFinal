...
<body>
    ...
    <div class="content-head">
        <h2>Find all your favorite movies here.</h2> 
        <p>Search for titles that you want to add to your favorites.</p>  
    </div> 
                ...
                if(isset($_GET["submit"])){
                    if(!empty($_GET["search"])){
                        ...
                        curl_setopt_array($curl, [
                            CURLOPT_URL => "https://online-movie-database.p.rapidapi.com/title/find?q=".$_GET["search"],
                        ...
                        if(count($results) > 0){
                            for($i = 0; $i < count($results) && $i < 20; $i++){
                                echo "<md>";
                                if(property_exists($results[$i], "image") && property_exists($results[$i]->image, "url")) echo '<img src="'.$results[$i]->image->url.'">';
                                echo "<movinf>".$results[$i]->title." (".$results[$i]->year.")</movinf>";
                                echo '<form action="" method="POST"><input type="submit" value="Add to Favorites" name="add_fav'.$results[$i]->id.'" /></form>';
                                echo "</md>";
                                if(isset($_POST["add_fav".$results[$i]->id])){
                                    try{
                                        mysqli_query($con, 'INSERT INTO favorite_movies VALUES("'.rtrim(substr($results[$i]->id, 7), "/").'", "'.$_SESSION['sess_user'].'")');
                                        unset($_POST["add_fav".$results[$i]->id]);
                            ...
                            echo 'No results for movies called "'.$_GET["search"].'"';
                        }
                    }
                }
            ?>
